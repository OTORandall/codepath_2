<?php
  ...
  function validate_country($country, $errors=array()) {
    ...
    } elseif( !is_valid_name($country['code'])){
      $errors[] = "Code can only be uppper case .";
  ...
  function validate_state($state, $errors=array()) {
    ...
    if (is_blank($state['code'])) {
      $errors[] = "Code cannot be blank.";
    } elseif( !is_valid_name($state['code'])){
      $errors[] = "Code can only be uppper case .";
    ...
    if (is_blank($state['country_id'])) {
      $errors[] = "Country_id cannot be blank.";
    } elseif( !is_valid_country_id($state['country_id'])){
      $errors[] = "country_id can only be numbers ";
    }
    return $errors;
  ...
  function validate_territory($territory, $errors=array()) {
    ...
    } elseif( !is_valid_position($territory['position'])){
      $errors[] = "Position can only be numbers .";
    }

    if (is_blank($territory['state_id'])) {
      $errors[] = "State_id cannot be blank.";
    } elseif( !is_valid_state_id($territory['state_id'])){
      $errors[] = "state_id can only be numbers ";
    }
    return $errors;
  }
  ...
  function insert_territory($territory) {
    ...
    $sql .= "'" . $name . "',";
    $sql .= "'" . $position . "',";
    // need to find the state_id prop in the new.php pages
    $sql .= "'" . $state_id . "'";
    $sql .= ");";
    //$sql = ""; // TODO add SQL
    // For INSERT statments, $result is just true/false
  ...
  function validate_salesperson($salesperson, $errors=array()) {
    ...
    } elseif( !is_valid_phone($salesperson['phone'])){
      $errors[] = "Phone can only be numbers ";
    }
    return $errors;
  }
  ...
  function insert_salesperson($salesperson) {
    ...
    $phone = db_escape($db, $salesperson['phone']);
  ...
  function update_salesperson($salesperson) {
    ...
    $phone = db_escape($db, $salesperson['phone']);
    $email = db_escape($db, $salesperson['email']);
    $id = db_escape($db, $salesperson['id']);
    ...
    // For update_salesperson statments, $result is just true/fales
    $result = db_query($db, $sql);
    if($result) {
      return true;
    } else {
  ...
  function validate_user($user, $errors=array()) {
    ...
    } elseif( !is_valid_name( $user['username'])){
      $errors[] = "Username can only be A-Z, a-z, 0-9, and _ .";
    }
  ...
  // custom delete user
  function delete_user($user, $errors=array()){
    global $db;
